...
describe('every variable an entrypoint reads is declared by its manifest', () => {
  ...
  it('covers the templates that ship one', () => {
    ...
    // become an empty suite that passes forever. 9router, openclaw and rakazo ship one while
    // requiring no credential at all, which is the case the per-template check below has to stay
    // honest about.
    expect(withEntrypoint.map((t) => t.dir).sort()).toEqual(['9router', 'claude-code', 'codex', 'dsh', 'hermes', 'openclaw', 'pi', 'rakazo']);
  });
