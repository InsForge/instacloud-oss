...
not. The platform has both a managed `redis` service and a portless `type: worker` service now,
and this template was built on them and then moved back: the managed Redis is TLS-only and hands
out a `rediss://` URL, while the two Redis clients Twenty constructs from that URL turn TLS on for
neither, so the server opens a connection, has it dropped, and retries forever without ever
logging an error. A worker on its own machine has no other Redis to meet the server at, so the two
stay together. The practical consequences: the three processes share one machine's CPU and memory,
a worker crash takes the whole service down and restarts it (the entrypoint exits non-zero on
purpose, so nothing is left running against a dead worker), and the instance does not scale to
