...
| `insforge` | `logo.svg` 1.0 KB | yes (vector) | **adapts** via `prefers-color-scheme` | InsForge's own mark, the two arrow paths of `assets/logo-dark.svg` in the upstream repository, taken at the 240x240 viewBox its README badge uses rather than from the 1000x240 wordmark lockup. Upstream draws the mark white on dark and black on light, with the same gradient in both files; this copy carries the gradient once and drives its stops from `currentColor`, so one file covers both |
