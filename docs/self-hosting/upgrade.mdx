...
```bash
pg_dump "$(insta db url)" > main.sql                                  # the branch's only Postgres
pg_dump "$(insta db url --branch feat)" > feat.sql
pg_dump "$(insta db url --group db --branch main)" > main-db.sql      # one line per service when there are several
pg_dump "$(insta db url --group analytics --branch main)" > main-analytics.sql
```
...
`/etc/instacloud/instad.env`). Treat the archive like a credential: it carries every secret, database
and certificate on the box, so encrypt any copy that leaves it.
...
psql "$(insta db url --group db)" < main-db.sql       # one line per Postgres service per branch
```
