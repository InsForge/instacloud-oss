...
and clients need to trust it. The same applies under `custom` when your certificate comes from a
private CA, with your own CA file in place of that path. Each client reads its own variable, so a
tool that is not told will fail on its own while everything else works:
