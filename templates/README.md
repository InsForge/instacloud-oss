...
| `gitea` | yes | **unbuilt** | Upstream's `1.27.3` index carries amd64, arm64 and riscv64, and the overlay only removes a directory and copies a shell script, so nothing in it is architecture-specific. The arm64 leg has NOT been run: this template was authored on a box with no docker daemon, and the sentence below this table is the standard this row does not yet meet. Run it before publishing |
