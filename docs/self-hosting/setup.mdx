...
`scp root@<box>:/var/lib/instacloud/edge/ca.pem ./instacloud-ca.pem`), then add
`--cacert ./instacloud-ca.pem` to the curls and `NODE_EXTRA_CA_CERTS=./instacloud-ca.pem` to the
