...
export type TabId = 'buckets' | 'database' | 'metrics' | 'variables' | 'deploys' | 'runtime' | 'volume' | 'settings'

export const TAB_LABELS: Record<TabId, string> = {
  buckets: 'Buckets', database: 'Database', metrics: 'Metrics', variables: 'Variables', deploys: 'Deployment Logs',
  runtime: 'Runtime Logs', volume: 'Volume', settings: 'Settings',
}
...
export function tabsFor(type: string): TabId[] {
  if (type === 'compute') return ['metrics', 'variables', 'deploys', 'runtime', 'volume', 'settings']
  if (type === 'postgres') return ['database', 'metrics', 'variables', 'deploys', 'runtime', 'settings']
  ...
  // Redis leads with the console's Database tab (the key browser); the daemon has no data browser
  // for mysql/mongodb yet, so those two keep the rail without it.
  if (type === 'redis') return ['database', 'metrics', 'variables', 'deploys', 'runtime', 'settings']
  if (MANAGED_TYPES.has(type)) return ['metrics', 'variables', 'deploys', 'runtime', 'settings']
